...
final class Recorder {
    ...
    private func startIO() {
        ...
        armWriterGateWatchdog()
    }

    // macOS holds a tap-containing aggregate until some process WRITES audio
    // to the output ("waiting for writers"): started on a silent Mac, the
    // device never cycles — mic included — with noErr everywhere and zero
    // frames. The launcher plays a silent kick at start; this watchdog covers
    // what the launcher can't: a kick that raced the device setup, and a
    // mid-recording rebuild (AirPods arriving during a quiet stretch) that
    // re-arms the gate.
    private func armWriterGateWatchdog() {
        ioQueue.async { self.sawCallback = false }
        ioQueue.asyncAfter(deadline: .now() + 3) { [weak self] in
            guard let self, !self.stopped, !self.sawCallback else { return }
            if self.gateKicks < 3, self.kickWriterGate() {
                self.gateKicks += 1
                log("IO has not started after 3s — macOS holds the tap until "
                    + "something plays audio; kicked it with the silent file "
                    + "(attempt \(self.gateKicks))")
                self.armWriterGateWatchdog()
            } else {
                log("WARNING: IO never started — if this persists, re-grant "
                    + "Microphone and 'System Audio Recording Only' in "
                    + "System Settings > Privacy & Security")
            }
        }
    }

    private func kickWriterGate() -> Bool {
        // <app support>/silence.wav, generated by the launcher; outputURL
        // lives one level down in .../com.stt-workflow/recordings/
        let wav = outputURL.deletingLastPathComponent().deletingLastPathComponent()
            .appendingPathComponent("silence.wav")
        guard FileManager.default.fileExists(atPath: wav.path) else { return false }
        let p = Process()
        p.executableURL = URL(fileURLWithPath: "/usr/bin/afplay")
        p.arguments = [wav.path]
    ...
    private func handle(_ abl: UnsafePointer<AudioBufferList>) {
        if !sawCallback {
            sawCallback = true
            log("IO running — first audio cycle arrived")
        }
        ...
        // the dead-side warning must run BEFORE the empty-cycle guard, or a
        // both-sides-dead capture (stale grants) never warns at all
        let n = max(nMic, nSys)
    ...
    private func rebuildForNewInput() {
        ...
        gateKicks = 0  // a fresh device gets a fresh writer-gate budget
        startIO()
    }
    ...
    func shutdown(code: Int32) {
        ioQueue.async { [self] in
            ...
            if framesWritten == 0 {
                log("WARNING: captured 0 frames — either nothing was playing " +
                    "sound (macOS holds tap captures until some app plays audio) " +
                    "or the grants are stale: Microphone + 'System Audio " +
                    "Recording Only' in System Settings > Privacy & Security")
            }
            exit(code)
        }
    }
